...
func (c *ClusterResourceBindingController) Reconcile(ctx context.Context, req controllerruntime.Request) (controllerruntime.Result, error) {
	...
	if !clusterResourceBinding.DeletionTimestamp.IsZero() {
		...
		if err := helper.DeleteWorks(ctx, c.Client, c.APIReader, "", req.Name, clusterResourceBinding.Labels[workv1alpha2.ClusterResourceBindingPermanentIDLabel], clusterResourceBinding.Spec.Resource, helper.ClustersThatMayHoldWorks(clusterResourceBinding.Spec, clusterResourceBinding.Status)); err != nil {
